...
//	}
//	aspect default{
//    	draw cylinder(2,6) at: location + {0, 7.5, 0} color: #brown ;
//    	draw cylinder(2,6) at: location + {0, -7.5, 0} color: #brown ;
//    }
//}
...
species Leader skills:[fipa]{
	...
	reflex createEncore when: deadStageCount >= 4 and encoreCreated=false
	{
		create encore
		{
...
//	aspect default
//	{
//		draw sphere(1) at: location color: #white ; 
//	}	
}
...
species Guest skills: [fipa, moving] {
	...
	reflex sendGuestsToStore when: deadStageCount >= 4
	{
		ask encore{
		//write "/////////////////"+ location;
		myself.preferredStage<- location;
		}
...
species Stage skills: [fipa, moving] {
	...
	reflex killStages when: time = 80
	{
		//do start_conversation with: [ to::list(Guest), protocol:: 'no-protocol', performative :: 'cfp', contents::['encore',ds.location]];
		deadStageCount <- deadStageCount +1;
		do die;
...
species encore skills:[fipa]
{
	rgb mycolor<- rnd_color(255);
	
	aspect default
	{
		draw cube (10) at: location color: #black;
